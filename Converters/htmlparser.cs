...
namespace Re_useable_Classes.Converters
{
    ...
    public class HtmlParser
    {
        ...
        internal static string AddHtmlClipboardHeader(string htmlString)
        {
            ...
            int endFragment = htmlString.IndexOf
                (
                    HtmlEndFragmentComment,
                    0,
                    StringComparison.Ordinal);
            if (endFragment >= 0)
            {
        ...
        {
            int startHtmlIndex = htmlDataString.IndexOf
                (
                    "StartHTML:",
                    StringComparison.Ordinal);
            if (startHtmlIndex < 0)
            ...
            int endHtmlIndex = htmlDataString.IndexOf
                (
                    "EndHTML:",
                    StringComparison.Ordinal);
            if (endHtmlIndex < 0)
            ...
            // TODO: We assume that indices represented by strictly 10 zeros ("0123456789".Length),
            // which could be wrong assumption. We need to implement more flrxible parsing here
            endHtmlIndex = Int32.Parse
                (
                    htmlDataString.Substring
                        (
                            endHtmlIndex + "EndHTML:".Length,
                            "0123456789".Length));
            ...
            return htmlDataString.Substring
                (
                    startHtmlIndex,
        ...
        private static void InvariantAssert
            (
            bool condition,
            string message)
        {
        ...
        private void CloseElement(string htmlElementName)
        {
            ...
            if (_pendingInlineElements.Count > 0 && _pendingInlineElements.Peek()
                                                                          .LocalName == htmlElementName)
            {
        ...
        {
            XmlElement htmlElementCopy = _document.CreateElement
                (
                    htmlElement.LocalName,
                    XhtmlNamespace);
            for (int i = 0;
                 i < htmlElement.Attributes.Count;
                 i++)
            {
                XmlAttribute attribute = htmlElement.Attributes[i];
                htmlElementCopy.SetAttribute
                    (
                        attribute.Name,
                        attribute.Value);
            }
        ...
        private void OpenStructuringElement(XmlElement htmlElement)
        {
            ...
            {
                while (_openedElements.Count > 0 && HtmlSchema.IsInlineElement
                                                        (
                                                            _openedElements.Peek()
                                                                           .LocalName))
                {
            ...
            if (_openedElements.Count > 0)
            {
                ...
                if (HtmlSchema.ClosesOnNextElementStart
                    (
                        htmlParent.LocalName,
                        htmlElement.LocalName))
                {
                    _openedElements.Pop();
                    htmlParent = _openedElements.Count > 0
                                     ? _openedElements.Peek()
                                     : null;
                }
        ...
        private XmlElement ParseHtmlContent()
        {
            ...
            XmlElement htmlRootElement = _document.CreateElement
                (
                    "html",
                    XhtmlNamespace);
                ...
                switch (_htmlLexicalAnalyzer.NextTokenType)
                {
                    case HtmlTokenType.OpeningTagStart:
                        ...
                        if (_htmlLexicalAnalyzer.NextTokenType == HtmlTokenType.Name)
                        {
                            ...
                            // Create an element
                            XmlElement htmlElement = _document.CreateElement
                                (
                                    htmlElementName,
                                    XhtmlNamespace);
