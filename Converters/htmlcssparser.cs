...
namespace Re_useable_Classes.Converters
{
    internal static class HtmlCssParser
    {
        ...
        private static readonly string[] Clears =
        {
            "none",
            "left",
            "right",
        ...
        private static readonly string[] FontRelativeSizes =
        {
            "larger",
            "smaller"
        };
        ...
        private static readonly string[] FontVariants =
        {
            "normal",
            "small-caps"
        };
        ...
        private static readonly string[] ListStylePositions =
        {
            "inside",
            "outside"
        };
        ...
        private static readonly string[] TextAligns =
        {
            "left",
            "right",
            "center",
        ...
        private static readonly string[] TextDecorations =
        {
            "none",
            "underline",
            "overline",
        ...
        private static readonly string[] TextTransforms =
        {
            "none",
            "capitalize",
            "uppercase",
        ...
        {
            string styleFromStylesheet = stylesheet.GetStyle
                (
                    elementName,
                    sourceContext);

            string styleInline = HtmlToXamlConverter.GetAttribute
                (
                    htmlElement,
                    "style");
            ...
            if (style != null)
            {
                ...
                {
                    string[] styleNameValue = t.Split(':');
                    if (styleNameValue.Length == 2)
                    {
                        string styleName = styleNameValue[0].Trim()
                                                            .ToLower();
                        string styleValue = HtmlToXamlConverter.UnQuote(styleNameValue[1].Trim())
                                                               .ToLower();
                        int nextIndex = 0;
        ...
        private static string ParseCssBorderStyle
            (
            string styleValue,
            ref int nextIndex)
        {
            return ParseWordEnumeration
                (
                    BorderStyles,
                    styleValue,
                    ref nextIndex);
        ...
        private static string ParseCssColor
            (
            string styleValue,
            ref int nextIndex)
        {
            ...
            if (nextIndex < styleValue.Length)
            {
                ...
                if (character == '#')
                {
                    ...
                    {
                        color = styleValue.Substring
                            (
                                startIndex,
                                nextIndex - startIndex);
                    }
                }
                else if (styleValue.Substring
                             (
                                 nextIndex,
                                 3)
                                   .ToLower() == "rbg")
                {
                    ...
                    color = RgbToHex(styleValue);
                }
                else if (Char.IsLetter(character))
                {
                    color = ParseWordEnumeration
                        (
                            Colors,
                            styleValue,
                            ref nextIndex);
        ...
        private static void ParseCssFont
            (
            string styleValue,
            Hashtable localProperties)
        {
            ...
            Hashtable localProperties)
        {
            ...
            {
                // Try generic-family
                string fontFamily = ParseWordEnumeration
                    (
                        FontGenericFamilies,
                        styleValue,
                        ref nextIndex);
                ...
                if (fontFamily == null)
                {
                    ...
                    if (nextIndex < styleValue.Length && (styleValue[nextIndex] == '"' || styleValue[nextIndex] == '\''))
                    {
                        ...
                        fontFamily = '"' + styleValue.Substring
                                               (
                                                   startIndex,
                                                   nextIndex - startIndex) + '"';
                    }
                ...
                if (fontFamily != null)
                {
                    ...
                    if (fontFamilyList == null && fontFamily.Length > 0)
                    {
                        if (fontFamily[0] == '"' || fontFamily[0] == '\'')
                        {
                            ...
                            fontFamily = fontFamily.Substring
                                (
                                    1,
                                    fontFamily.Length - 2);
                        }
        ...
        private static void ParseCssListStyle
            (
            string styleValue,
            Hashtable localProperties)
        {
            ...
            while (nextIndex < styleValue.Length)
            {
                string listStyleType = ParseCssListStyleType
                    (
                        styleValue,
                        ref nextIndex);
                ...
                {
                    string listStylePosition = ParseCssListStylePosition
                        (
                            styleValue,
                            ref nextIndex);
                    ...
                    {
                        string listStyleImage = ParseCssListStyleImage
                            (
                                styleValue,
                                ref nextIndex);
        ...
        private static string ParseCssListStyleImage
            (
            string styleValue,
            ref int nextIndex)
        {
        ...
        private static string ParseCssListStylePosition
            (
            string styleValue,
            ref int nextIndex)
        {
        ...
        private static string ParseCssListStyleType
            (
            string styleValue,
            ref int nextIndex)
        {
            ...
            string propertyName)
        {
            ...
            Debug.Assert
                (
                    propertyName == "margin" || propertyName == "padding" || propertyName == "border-width" ||
            ...
            bool mustBeNonNegative)
        {
            ...
            if (nextIndex < styleValue.Length && Char.IsDigit(styleValue[nextIndex]))
            {
                ...
                string number = styleValue.Substring
                    (
                        startIndex,
                        nextIndex - startIndex);

                string unit = ParseWordEnumeration
                                  (
                                      FontSizeUnits,
                                      styleValue,
                                      ref nextIndex) ?? "px";
            ...
            Hashtable localProperties)
        {
            ...
            while (nextIndex < styleValue.Length)
            {
                string decoration = ParseWordEnumeration
                    (
                        TextDecorations,
                        styleValue,
                        ref nextIndex);
        ...
        {
            string attributeValue = ParseWordEnumeration
                (
                    words,
                    styleValue,
                    ref nextIndex);
        ...
        private static string RgbToHex(String rgbColor)
        {
            try
            {
                rgbColor = rgbColor.ToLower();
                ...
                rgbColor = rgbColor.Replace
                    (
                        ")",
                        "");
                string[] arrStr = rgbColor.Split(',');

                int red = Int32.Parse(arrStr[0]);
                int green = Int32.Parse(arrStr[1]);
                int blue = Int32.Parse(arrStr[2]);
